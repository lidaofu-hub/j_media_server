...
public class MKNoReaderCallBack implements IMKNoReaderCallBack {
    ...
    public void invoke(MK_MEDIA_SOURCE sender) {
        String appValue = MediaServerContext.ZLM_API.mk_media_source_get_app(sender);
        String streamValue = MediaServerContext.ZLM_API.mk_media_source_get_stream(sender);
        String schemaValue = MediaServerContext.ZLM_API.mk_media_source_get_schema(sender);
